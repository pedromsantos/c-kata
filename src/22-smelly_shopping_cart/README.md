...
objective as `21-smelly_mars_rover`, but with a more everyday domain: a
shopping cart (inspired by `23-shopping_cart`), built across the same
...
Unlike `21-smelly_mars_rover`, which is a single kata.c/kata.h pair, this
kata is laid out across the three layers of a small hexagonal application
(`Domain/`, `Application/`, `Infrastructure/`) so you can practice all three
test types on the same codebase: acceptance, unit, and integration. Ports
(`Clock`, `NotificationPort`, `ShoppingCartRepository`) are structs of
function pointers; adapters (`OrderClock`, `EmailNotificationGateway`,
`InMemoryShoppingCartRepository`) are structs that embed the port struct as
their first member and wire matching function pointers into it. See
`PORTING_NOTES_C.md` in `ts-kata-answers/17_SmellyShoppingCart` for more on
this pattern and its limitations versus the TypeScript version.

None of the `.h`/`.c` files in this kata contain explanatory comments about
*where* the problems are. Finding and naming each blocker and each smell is
part of the exercise — this README intentionally does not say where they
are.
...
| Domain (services)   | tested              | tests exist in `test_kata.c`, but nearly all are smelly                 |
| Infrastructure      | tested              | tests exist in `test_kata.c`, but nearly all are smelly                 |
...
testability blockers — things that make a function or struct hard or
impossible to put under test as-is. Before writing a single test, identify
each one and name it using a legacy-code testability taxonomy of your
choice (constructor/factory function does real work, hardcoded dependency,
static/global state, "static"-style function with a side effect,
non-determinism with no seam, etc.). There are more blockers than in
`21-smelly_mars_rover`, spread across two layers.
...
coverage — look closely at *which* of its functions are actually exercised,
and by what.
...
2. **Fix the existing unit tests** — everything in `test_kata.c`'s unit
   groups currently passes, but almost none of it protects real behavior.
   Detect each smell, then refactor it without ever weakening or deleting a
   test to make it pass.
3. **Fix the existing integration tests** — same exercise, but for the
   integration groups. Pay attention to how the same smell can look
...
cmake -B build
cmake --build build
./build/test_22-smelly_shopping_cart
```
