...
```c
const char* to_roman(int number);
```
...
| --- | --------------------------- | ---------------- | ------------------------------------ |
| 1   | {} → nil                    | {}               | return NULL                          |
| 2   | nil → constant              | return NULL      | return "1"                           |
...
| 10  | if → loop                   | if(condition)    | while(condition)                     |
...
| 3     | 2               | unconditional → conditional | `if (number < 2) return index; else return index - 1;` NO CHANGE        |
| 4     | 3               | unconditional → conditional | `if (number < 2) return index; else return index - 1;` NO CHANGE        |
| 5     | 5               | scalar → array              | `int fibs[] = {0, 1, 1, 2, 3, 5}; return fibs[index];`                  |
| 6     | 8               | scalar → array              | `int fibs[] = {0, 1, 1, 2, 3, 5, 8}; return fibs[index];`               |
| 7     | 13              | scalar → array(duplication) | `int fibs[] = {0, 1, 1, 2, 3, 5, 8, 13}; return fibs[index];`           |
