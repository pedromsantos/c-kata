...
This kata ports ts-kata's `api.ts`, which is itself only a 226-line stub: an
Express transport layer whose route handlers all return hardcoded
placeholder values (behind `// TODO: Implement ... logic` comments) instead
of consulting any real domain model. `kata.h`/`kata.c` port that same
completion level: the domain types (`Player`, `Location`, `Item`, `Bag`,
`Direction`, `Action`) plus a `KatacombsPorts` struct of function pointers —
one per use case — whose default implementation
(`katacombs_ports_create_stub`) returns the same canned values api.ts did.
There is no transport layer here at all (not even a stub one), per the
project requirement above.

The kata is to replace each port function with a real implementation,
backed by a `PlayerRepository` and a `WorldRepository` you design (as
function-pointer ports of their own, following the pattern used in
`22-smelly_shopping_cart`), and to grow `test_kata.c` from characterizing
the placeholder values into real unit/acceptance tests of the game rules
below.
